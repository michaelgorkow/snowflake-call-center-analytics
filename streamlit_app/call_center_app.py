...
# Constants
SEMANTIC_MODEL = 'CALL_CENTER_DEMO_DB.PUBLIC.CALL_CENTER_MODEL'
...
def query_cortex_analyst_rest_api(question: str, conversation_history: List[Dict] = None) -> Dict[Any, Any]:
    ...
    try:
        ...
        request_body = {
            "messages": messages,
            "semantic_models": [{'semantic_view':SEMANTIC_MODEL}],#semantic_model_yaml,  # Pass YAML content directly
...
def upload_audio_file_to_stage(uploaded_file):
    ...
    try:
        ...
        stage_path = f"@AUDIO_FILES/audio_files/{today}/{uploaded_file.name}"
        ...
        return True, f"audio_files/{today}/{uploaded_file.name}"
...
def show_stage_files():
    ...
    try:
        ...
        stage_path = f"@AUDIO_FILES/audio_files/{today}/"
        ...
        try:
            ...
            if result and len(result) > 0:
                ...
                for i, row in enumerate(result[:10]):  # Show max 10 files
                    ...
                    full_stage_path = f"@AUDIO_FILES/audio_files/{today}/{file_name}"
...
def try_match_verified_query(user_input: str):
    ...
    try:
        session = get_snowpark_session()
        r = session.sql('DESCRIBE SEMANTIC VIEW CALL_CENTER_DEMO_DB.PUBLIC.CALL_CENTER_MODEL').filter(F.col('"object_kind"') == 'EXTENSION').select('"property_value"').collect()[0][0]
        verified_queries = json.loads(r)['verified_queries']
...
def overview_dashboard():
    ...
    with st.container(border=True):
        ...
        with col2:
            total_duration = all_audio_calls["TRANSCRIPTION_DURATION_SECONDS"].sum() / 60  # Convert to minutes
        ...
        if audio_files_list:
            ...
            if st.session_state.selected_action is None and st.session_state.get('audio_processed', False):
                ...
                if audio_file_name:
                    try:
                        ...
                        if call_date:
                            ...
                            stage_path = f"@AUDIO_FILES/audio_files/{date_str}/{audio_file_name}"
                ...
                with info_col1:
                    ...
                    st.write(f"• **Duration:** {selected_call.get('TRANSCRIPTION_DURATION_SECONDS', 'N/A')} seconds")
                    
                    st.markdown("**🎙️ Call Diarization:**")
                    diarization = selected_call.get('CONVERSATION_STRUCTURED', 'N/A')
        ...
        with st.container(border=True):
            ...
            WITH monthly_data AS (
                SELECT 
                    ...
                    COUNT(*) / COUNT(DISTINCT DATE(DATE)) as avg_daily_calls
                FROM PUBLIC.ANALYZED_TRANSCRIPTIONS_APP
